        ...
        <el-form :inline="true" :model="comForm" ref="form">
          ...
          <div class="twoRadio">
            <el-radio-group v-model="radio2">
              <el-radio :label="1" @change="comSumKpi">总KPI</el-radio>
              <el-radio :label="2" class="radioTwo" @change="comAvgKpi">平均KPI</el-radio>
            </el-radio-group>
          </div>
          <!-- 查询按钮 -->
          <div class="seachButton">
        ...
        <el-form :inline="true" :model="depForm" ref="form">
          ...
          <div class="twoRadio">
            <el-radio-group v-model="radio4">
              <el-radio :label="1" @change="depSumKpi">总KPI</el-radio>
              <el-radio :label="2" class="radioTwo" @change="depAvgKpi">平均KPI</el-radio>
            </el-radio-group>
          </div>
          <!-- 查询按钮 -->
          <div class="seachButton">
        ...
        <el-form :inline="true" :model="empForm" ref="form">
          ...
          <div class="twoRadio">
            <el-radio-group v-model="radio6">
              <el-radio :label="1" @change="empSumKpi">总KPI</el-radio>
              <el-radio :label="2" class="radioTwo" @change="empAvgKpi">平均KPI</el-radio>
            </el-radio-group>
          </div>
          <!-- 查询按钮 -->
          <div class="seachButton">
...
export default {
  ...
  methods: {
    ...
    async getGroupList() {
      ...
      this.groupInitItem(AllXTimes, groupNetWorth,groupBonusPoints,groupDemerit);
    },
    // 集团kpl趋势折线图
    groupInitItem(AllXTimes = "", groupNetWorth = "", groupBonusPoints = "", groupDemerit = "") {
      // 集团的图表
    ...
    async fuzzComList() {
      ...
      const { data } = await querySearchCompany(value);
      const { fuzzyCompanyListList } = data;
      this.fuzzCom = fuzzyCompanyListList[0]
      this.fuzzComList2()

    },
    // 公司折线图函数数据
    async fuzzComList2(index=0) {
      const value = {
        company: this.fuzzCom,
      ...
      // 员工净值
      const comNetWorth = dateEntityList.map(item => {
        return item.KPI;
      });
      // 员工加分
      const comBonusPoints = dateEntityList.map(item => {
        return item.KPIjiafen;
      });
      // 员工减分
      const comDemerit = dateEntityList.map(item => {
        return item.KPIjianfen;
      });
      this.comInitItem(AllXTimes, comNetWorth,comBonusPoints,comDemerit);

    },
    // ---
    async getComList(index=0) {
      const value = {
        company: this.comForm.comCompany,
        actionBeginTime: this.comForm.comStartTime,
        actionEndTime: this.comForm.comEndTime,
    ...
    // 公司kpl趋势折线图
    comInitItem(AllXTimes = "", comNetWorth = "", comBonusPoints = "", comDemerit = "") {
      this.initWidth = document.getElementById("ttt").offsetWidth;
    ...
    async fuzzDepList () {
      ...
      const { data } = await queryDownListGroup(value);
      const { dateEntityList } = data;
      const { fuzzyEmployeeGroupList } = data;
      this.fuzzGroup = fuzzyEmployeeGroupList[0]
      this.fuzzDepList2()
    },
    async fuzzDepList2 (index=0) {
      const value = {
        company: this.depForm.depCompany,
        employeeGroup: this.fuzzGroup,
      ...
      // 员工净值
      const depNetWorth = dateEntityList.map(item => {
        return item.KPI;
      });
      // 员工加分
      const depBonusPoints = dateEntityList.map(item => {
        return item.KPIjiafen;
      });
      // 员工减分
      const depDemerit = dateEntityList.map(item => {
        return item.KPIjianfen;
      });
      this.depInitItem(AllXTimes, depNetWorth,depBonusPoints,depDemerit);
    },
    // ---
    async getDepList (index=0) {
      const value = {
        company: this.depForm.depCompany,
        employeeGroup: this.depForm.depDepartment,
        actionBeginTime: this.depForm.depStartTime,
    ...
    // 部门kpl趋势折线图
    depInitItem(AllXTimes = "", depNetWorth = "", depBonusPoints = "", depDemerit = "") {
      this.initWidth = document.getElementById("ttt").offsetWidth;
    ...
    // 填充员工kpl趋势折线图
    mepInitItem(AllXTimes = "", empNetWorth = "", empBonusPoints = "", empDemerit = "") {
      this.initWidth = document.getElementById("ttt").offsetWidth;
